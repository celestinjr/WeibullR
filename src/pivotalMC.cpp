...
arma::rowvec getRAIpiv(Rcpp::NumericVector dq,
                       unsigned int S,
                       arma::mat qpiv) {
  double ci;
  int LCB;
  arma::rowvec RAIpiv(dq.length());

  for(int i=0; i<dq.length(); i++) {
    ci = abs(1-2*dq[i]);
    LCB=(int) S*(1-ci)/2;
    if (dq[i] < 0.5) {
      RAIpiv[i] = qpiv(LCB-1, i);
    } else if (dq[i] == 0.5) {
      RAIpiv[i] = qpiv(S/2-1, i);
    } else {
      RAIpiv[i] = qpiv(S-LCB-1, i);
    }
  }
  return RAIpiv;
}
...
// this is the unlikely case that both extended output objects are called for
	...
	case 8:
		return DataFrame::create(
			...
			Rcpp::Named("RAI")=wrap(arma::trans(RAIpiv)) );
		break;
